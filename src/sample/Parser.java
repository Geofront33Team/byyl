...
public class Parser {
	...
	public void block(int lev, BitSet followSys) {
		...
		int dataSize0 = this.dataSize, // 记录本层之前的数据量,以便返回时恢复
	...
	void statement(BitSet followSys, int lev) {
		...
		switch (sym.symtype) {
			case Symbol.ident:
				parseAssignStatement(followSys, lev);
				break;
			case Symbol.readsym:
				parseReadStatement(followSys, lev);
				break;
			case Symbol.writesym:
				parseWriteStatement(followSys, lev);
				break;
			case Symbol.callsym:
				parseCallStatement(followSys, lev);
				break;
			case Symbol.ifsym:
				parseIfStatement(followSys, lev);
				break;
			case Symbol.beginsym:
				parseBeginStatement(followSys, lev);
				break;
			case Symbol.whilesym:
				parseWhileStatement(followSys, lev);
				break;
			case Symbol.repeatsym:
				parseRepeatStatement(followSys, lev);
				break;
			default:
	...
	private void parseRepeatStatement(BitSet followSys, int lev) {
		nextsym();
		...
		if (sym.symtype == Symbol.untilsym) {
			...
			//myErr.report(this.dataSize);
			System.out.println("Parser 300行的错误处理");
		}
	}

	//<当型循环语句> <当型循环语句> ::= while<条件>do<语句>
	private void parseWhileStatement(BitSet followSys, int lev) {
		nextsym();
		BitSet nxtlev = (BitSet) followSys.clone();
	...
	private void parseBeginStatement(BitSet followSys, int lev) {
		nextsym();
		BitSet nxtlev = (BitSet) followSys.clone();
		nxtlev.set(Symbol.semicolon);
		nxtlev.set(Symbol.endsym);
	...
	private void parseIfStatement(BitSet followSys, int lev) {
		nextsym();
		BitSet nxtlev = (BitSet) followSys.clone();
	...
	//<标识符> <过程调用语句> ::= call<标识符>
	private void parseCallStatement(BitSet followSys, int lev) {
		nextsym();
		if (sym.symtype == Symbol.ident) {
	...
	private void parseWriteStatement(BitSet followSys, int lev) {
		nextsym();
	...
	private void parseReadStatement(BitSet followSys, int lev) {
		nextsym();
		if (sym.symtype == Symbol.lparen) {
			int index = 0;
			do {
	...
	//:=<表达式> <赋值语句> ::= <标识符>:=<表达式>
	private void parseAssignStatement(BitSet followSys, int lev) {
